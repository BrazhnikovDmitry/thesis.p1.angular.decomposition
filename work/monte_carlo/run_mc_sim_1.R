...
for (i in 1:NUM_trials) {
     ...
     angs <- alphas + runif(NUM_waves, min = -1, max = 1)*dang#rep(1, NUM_waves)#
     amps <- runif(NUM_waves, min = 0, max = 1)#rep(1, NUM_waves)#
     phases <- rep(0, NUM_waves)#runif(NUM_waves, min = 0, max = 2*pi)#
     all_phases[i, ] <- phases
     all_amps[i, ] <- amps
     print(amps)
     print(phases)
     ...
     antenna2 <-
          generate_circ_antenna(
               ...
               wavenum,
               num_p = c(24, 12, 0),
               radius = c(1.2 * lambda / 2, 0.5 * lambda / 2, lambda / 2),
               shift_rad = c(-pi / 5, 0, pi / 5)
          )
     ...
     err1[[i]] <- calc_errors_1(angs, amps, dang, dirs_spectra, Sf_out1[[i]]$Sf)
     err2[[i]] <- calc_errors_1(angs, amps, dang, dirs_spectra, Sf_out2[[i]]$Sf)

     m.png(paste0("dump_pix_mc_1/", i, ".png"), w = 15, h = 8)
     par(mfrow = c(1, 1))
