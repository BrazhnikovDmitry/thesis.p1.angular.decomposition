              ...
              rm_zeros = TRUE)
{
     ...
     coef_u_vel <- 1#max(abs(dtc_p)) / max(abs(c(dtc_u, dtc_v)))
     ...
     # clean all zero rows corresponding to quad-spectra == 0
     # if (rm_zeros)  {
     #      idx <- which(A[, 1] == 0)
     #      idx2rm <- integer(0)
     #      for (deldx in idx) if (all (A[deldx, ] == 0)) idx2rm <- c(idx2rm, deldx)
     # 
     #      b <- b[-idx2rm]
     #      A <- A[-idx2rm, ]
     # }
